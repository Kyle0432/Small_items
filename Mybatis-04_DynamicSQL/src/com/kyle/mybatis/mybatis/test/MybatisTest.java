...
public class MybatisTest {
	...
	public void testBatchSave() throws IOException {
		...
		try {
		    ...
		    emps.add(new Employee(null,"alive","alive@qq.com","0",new Department(1)));
		    mapper.addEmps(emps);
		}finally{
			openSession.close();
		}
	...
	public void testDynamicSql() throws IOException {
		...
		try {
			EmployeeMapperDynamicSQL mapper = openSession.getMapper(EmployeeMapperDynamicSQL.class);
            Employee employee = new Employee(3,"caven",null,null);
//            List<Employee> emps = mapper.getEmpsByConditionIf(employee);
//			for(Employee employee2:emps){
//				System.out.println(employee2);
//			}
			//查询的时候如果某些条件没带可能sql拼装会出问题,
			//比如where后面的第一个条件没有而第一和第二个是用and连接的
			//所以第一条件为null就会出现一个where and[第二个条件]的错误问题
			//解决方式:
...
//				}
            ...
            //测试choose
//            List<Employee>  emp3 = mapper.getEmpsByConditionChoose(employee);
//            for(Employee emp : emp3){
//            	System.out.println(emp);
//            }
            //测试set标签
//            mapper.updateEmp(employee);
            //更新完需要提交下,或者在上的sqlSessionFactory.openSession()里面加true
//            openSession.commit();
            
            //测试foreach
            List<Employee> employees  = mapper.getEmpsByConditionForeach(Arrays.asList(1,3,4));
